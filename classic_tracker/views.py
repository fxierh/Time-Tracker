...
from django.db.models import Max, QuerySet, OuterRef, Subquery, Count, F
...
class HomeView(TemplateView):
    template_name = 'classic_tracker/home.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            Day.objects.filter(user=self.request.user.id).aggregate(
                max_usable_time=Max('usable_time'),
                max_study_time=Max('study_time'),
                max_time_usage_ratio=Max('time_usage_ratio')
            )
        )
...
class DayListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if stage:
            ...
            return Day.objects\
                .filter(user=self.request.user.id, stage=stage)\
...
class DayDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context['sessions'] = Session.objects.filter(day=day_obj).all()
