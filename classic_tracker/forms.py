...
from django.forms import ModelForm, NumberInput, TimeInput, \
    TextInput, Textarea, DateInput, MultiWidget
...
class DurationSelector(MultiWidget):
    ...
    def __init__(
        ...
        # }
        widgets = {
            'hour': NumberInput(attrs={'min': min_hour, 'max': max_hour, 'step': step_hour, 'placeholder': 'hh'}),
            'minute': NumberInput(
                attrs={'min': min_minute, 'max': max_minute, 'step': step_minute, 'placeholder': 'mm'}
            ),
        }
...
class DayCreateUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            'start': 'Start time',
            'end': 'End time',
            'end_next_day': 'Ends later than or at midnight',
        }
        ...
        widgets = {
            ...
            # Sets default value for the start field
            'start': TimeSelector,
            'end': TimeSelector,
            'comment': Textarea(attrs={'wrap': 'soft'})
        }
...
class SessionCreateUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            'start': 'Start time',
            'end': 'End time',
            'end_next_day': 'Ends later than or at midnight (and starts before midnight)',
        }
...
class StageCreateUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        # Customize widget for attribute
        widgets = {
            'name': TextInput(attrs={'size': 13}),
            'description': Textarea(attrs={'wrap': 'soft'})
        }
